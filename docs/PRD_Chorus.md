...
| **PM 专属** | | |
| `chorus_pm_create_proposal` | 创建 Proposal 容器（draft 状态） | PM |
| `chorus_pm_submit_proposal` | 提交 Proposal 审批（draft → pending） | PM |
| `chorus_pm_add_document_draft` | 向 Proposal 添加文档草稿 | PM |
| `chorus_pm_update_document_draft` | 更新 Proposal 中的文档草稿 | PM |
| `chorus_pm_remove_document_draft` | 删除 Proposal 中的文档草稿 | PM |
| `chorus_pm_add_task` | 向 Proposal 添加任务（含验收标准） | PM |
| `chorus_pm_update_task` | 更新 Proposal 中的任务 | PM |
| `chorus_pm_remove_task` | 删除 Proposal 中的任务 | PM |
| `chorus_pm_create_document` | 直接创建文档（不通过 Proposal） | PM |
| `chorus_pm_create_tasks` | 直接批量创建任务（不通过 Proposal） | PM |
| `chorus_pm_update_document` | 更新已创建的文档内容 | PM |
...
| **Admin 专属（代理人类操作）** | | |
| `chorus_admin_create_project` | 创建新项目 | Admin |
| `chorus_admin_create_idea` | 创建 Idea（代理人类提出需求） | Admin |
| `chorus_admin_approve_proposal` | 审批 Proposal（pending → approved） | Admin |
| `chorus_admin_reject_proposal` | 拒绝 Proposal（pending → rejected） | Admin |
| `chorus_admin_verify_task` | 验证 Task（to_verify → done） | Admin |
| `chorus_admin_reopen_task` | 重新打开 Task（to_verify → in_progress） | Admin |
| `chorus_admin_close_task` | 关闭 Task（any → closed） | Admin |
| `chorus_admin_delete_content` | 删除任意内容（Idea/Task/Document/Proposal） | Admin |
...
> ⚠️ **注意**: 下方 schema 为概念参考，实际实现已迁移到 **UUID-based 架构**（所有外键引用使用 `xxxUuid` 字段而非数字 ID）。
> 请参考 `prisma/schema.prisma` 获取最新的数据模型定义。

```prisma
// prisma/schema.prisma (概念参考，实际以源码为准)
...
model Agent {
  ...
  company   Company  @relation(fields: [companyId], references: [id])
  name      String
  roles     String[] @default(["developer"])  // pm | developer | admin（可多选）
...
model Task {
  ...
  acceptanceCriteria String? // 验收标准（Markdown 格式，新增）
  // 状态与认领
...
model Proposal {
  ...
  createdBy   Int      // Agent ID 或 User ID
...
│   │   ├── SKILL.md
│   │   └── HEARTBEAT.md
│   └── admin/                  # Admin Agent 专用（代理人类）
│       ├── SKILL.md            # 审批/验证/项目管理操作指南
│       └── HEARTBEAT.md
└── .env.example
```
