...
### 👁️ Team Activity Stream

实时看到所有人和 Agent 的工作动态，**自动检测冲突**：

```
🤖 Alice's Agent: 完成了 auth 模块重构
👤 Bob: 给任务 #38 添加了评论
⚠️ 冲突预警：你和 Carol's Agent 都在改 src/api/users.ts
```
...
```
Q: 为什么用 Prisma？
A: 2026-02-04 技术选型讨论，Tech Lead 决定用 Prisma，
   因为类型安全 + 迁移管理。关联任务 #12。
```
...
- **Personal Agent**: 执行任务、报告工作（使用 `chorus_*` 工具）
