...
This changes the status from `draft` to `pending`. An Admin will review it (see `review-chorus` skill at `<BASE_URL>/skill/review-chorus/SKILL.md`).

Consider adding a comment explaining your reasoning:
...
- Consider breaking large features into multiple smaller tasks rather than one monolithic task
- Add `storyPoints` to help prioritize and estimate effort
- Use `acceptanceCriteria` with checkboxes for clear verification
- Prefer setting up task dependency DAG — tasks without dependencies are assumed parallelizable
- When combining multiple ideas, consider explaining how they relate in the proposal description

---

## Next

- After submission, an Admin will review using the `review-chorus` skill (`<BASE_URL>/skill/review-chorus/SKILL.md`)
- After approval, Developers claim tasks using the `develop-chorus` skill (`<BASE_URL>/skill/develop-chorus/SKILL.md`)
- For Idea elaboration, see the `idea-chorus` skill (`<BASE_URL>/skill/idea-chorus/SKILL.md`)
- For platform overview, see the `chorus` skill (`<BASE_URL>/skill/chorus/SKILL.md`)
